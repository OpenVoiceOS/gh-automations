...
| `soxr` | `LGPL-2.1-or-later` | WeakCopyleft | The `python-soxr` wheel ships the `libsoxr` shared library. LGPL-2.1 places its conditions on a program that is **distributed** while linking the library: it must allow the user to replace it. We install `soxr` from PyPI at run time, link it dynamically, modify no source, and redistribute neither the library nor any binary containing it, so no conveying happens and the reciprocal clause is never reached. Pulled transitively through `librosa`, which uses it for resampling. |
| `nvidia-*` | proprietary (NVIDIA End User License Agreement) | Other | The CUDA runtime wheels (`nvidia-cudnn-cu12`, `nvidia-cublas-cu12` and the rest of the family; `torch` names a different subset per release, so the entry covers the family). They are pulled by `torch` on Linux and installed on the GPU host that runs the training or inference. We redistribute none of them: the licence governs that host's own use of NVIDIA software, and nothing we ship contains it. The category is correct and the use is outside what it restricts. |
| `cuda-toolkit` | proprietary (NVIDIA End User License Agreement) | Other | Same footing as the `nvidia-*` wheels: a CUDA build dependency installed on a GPU host at run time, never redistributed. |
